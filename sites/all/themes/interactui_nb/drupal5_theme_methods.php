...
function interactui_nb_regions() {
  return array(
 ...
 * @ingroup themeable
 */
function interactui_nb_menu_local_task($mid, $active, $primary) {
  $active_class = "";
  if ($active) {
    $active_class .= "active ";
  }
  $link = menu_item_link($mid, FALSE);
  $output = '<span class="'.$active_class.'gs-button-wrapper">'.
    '<span class="gs-button-l"></span>'.
    '<span class="gs-button-r"></span>'.
    '<a href="?q='.$link['href'].'" class="'.$active_class.'gs-button">'.$link['title'].'</a></span>';
  return '<li>'.$output.'</li>';
}
 ...
 *   The url of the feed.
 */
function interactui_nb_feed_icon($url) {
  return '<a href="'. check_url($url) .'" class="gs-rss-tag-icon" title="' . t('Syndicate content') . '"></a>';
}

/**
 * Allow themable wrapping of all comments.
 */
function interactui_nb_comment_wrapper($content, $type = null) {
  static $node_type;
...
<div class="gs-box gs-post">
  ...
  if ($content && $node_type != 'forum') {
    $result .= '<h2 class="gs-postheader comments">' . t('Comments') . '</h2>';
  }
 ...
 */
function interactui_nb_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
    return '<div class="breadcrumb gs-postcontent">'. implode(' | ', $breadcrumb) .'</div>';
  }
}

function interactui_nb_service_links_node_format($links) {
 ...
 * @ingroup themeable
 */
function interactui_nb_button($element) {
  // Make sure not to overwrite classes.
  if (isset($element['#attributes']['class'])) {
    $element['#attributes']['class'] = 'form-'.$element['#button_type'].' '.$element['#attributes']['class'].' gs-button';
  }
  else {
    $element['#attributes']['class'] = 'form-'.$element['#button_type'].' gs-button';
  }
   	
  return '<span class="gs-button-wrapper">'.
    '<span class="gs-button-l"></span>'.
    '<span class="gs-button-r"></span>'.
...
*/
function interactui_nb_img_assist_page($content, $attributes = NULL) {
