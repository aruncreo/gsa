...
function interactui_nb_menu_local_task($link, $active = FALSE) {
  $active_class = "";
  if ($active) {
    $active_class .= "active ";
  }
  $output = preg_replace('~<a href="([^"]*)"[^>]*>([^<]*)</a>~',
    '<span class="'.$active_class.'gs-button-wrapper">'.
    '<span class="gs-button-l"></span>'.
    '<span class="gs-button-r"></span>'.
    '<a href="$1" class="'.$active_class.'gs-button">$2</a></span>', $link);
  return '<li>'.$output.'</li>';
}
 ...
 *   A descriptive title of the feed.
  */
function interactui_nb_feed_icon($url, $title) {
  return '<a href="'. check_url($url) .'" class="gs-rss-tag-icon" title="' . $title . '"></a>';
}

/**
 * Add a "Comments" heading above comments except on forum pages.
 */
function interactui_nb_preprocess_comment_wrapper(&$vars) {
 ...
 * @ingroup themeable
 */
function interactui_nb_node_preview($node) {
