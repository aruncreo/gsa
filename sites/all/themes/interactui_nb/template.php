...
/**
 * Implements file_formatter_table() for adding File Type to the Document Table view
 */
function interactui_nb_file_formatter_table($variables) {
	if ($variables['items'][0]['type'] == 'document')	{
		$header = array(t('Attachment'), t('Size'), t('Type'));
		$rows = array();
		foreach ($variables['items'] as $delta => $item) {
			$file_type_document = explode(".",$item['filename']);
			$rows[] = array(
				theme('file_link', array('file' => (object) $item)),
				format_size($item['filesize']),
				strtoupper(end($file_type_document)),
			);
		}
