...
function art_menu_xml_parcer($content, $show_sub_menus, $menu_class) {
  ...
  foreach($ul_elements as $ul) {
    // First ul element with css-class gs-hmenu or gs-vmenu
    if (($ul->getAttribute('class') == "gs-vmenu") || ($ul->getAttribute('class') == "gs-hmenu"))
        break;
    continue;
  }
...
function art_hmenu_output($content) {
  $menu = art_menu_worker($content, true, 'gs-hmenu');
...
function art_vmenu_output($subject, $content) {
  ...
  $result = '';
  $vmenu =	art_menu_worker($content, false, 'gs-vmenu');
